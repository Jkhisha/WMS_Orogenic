...
{ 
    public class HandOverByRepository : IHandOverByRepository
    {
        ...
        public void InsertOrUpdate(HandOverBy handoverby)
        {
            ...
            {
                var existingEntity = context.HandOverBies.Find(handoverby.HandOverById);

                if (existingEntity != null)
                {
                    context.Entry(existingEntity).CurrentValues.SetValues(handoverby);
                }
                else
                {
                    context.HandOverBies.Attach(handoverby);
