...
{ 
    public class BarCodeRepository : IBarCodeRepository
    {
        ...
        public BarCode GetBarcodeTxtEmptyBoxes(long EmtBxId)
        {
            ...
            var path = "D:\\WHMS2\\Content\\Images\\BarCodes\\" + "AssignBox-" + EmtBxId.ToString();
        ...
        public ReportViewModelForBarCode GetEmptyBoxBarCodeReport(List<EmptyBoxBarcode> barcodesList)
        {
            var _barCodeList = new List<object>(); // Define the object list

            // Populate the list with barcode data
            foreach (var barcode in barcodesList)
            {
                _barCodeList.Add(new
                {
                    BarCodeText = barcode.BarCodeText,
                    EmptyBoxId = barcode.EmptyBoxId
