...
{
    public class BoxLocationsController : BaseController
    {
        ...
        public ActionResult FileSearch()
        {
            ...
            ViewBag.ClientList = repo.ClientRepository.AllIncluding();
            ViewBag.PossibleDepartments = repo.DepartmentRepository.AllIncluding();
            model.ListItems = repo.AssignBoxRepository.GetAllFileByClientId(clientId);

            List<BoxSearchListViewModelFile> bosSearchList = new List<BoxSearchListViewModelFile>();

            List<AssignBox> assignBoxList = repo.AssignBoxRepository.GetAllFileByClientId(clientId);
        ...
        public ActionResult FileSearch(BoxSearchViewModel model)
        {
           ...
           // long _clientID = model.ClientID;
            long _deptID = model.DepartmentID.Value;
            DateTime _startDate = DateTime.Now;
            DateTime _endDate = DateTime.Now;
            ...
            listofAllBoxes = repo.AssignBoxRepository.GetByClientIdAndDepartmentIdFile(clientId, _deptID);
        ...
        public ActionResult BoxInfoByBarcode(BarcodeInfoSearchViewModel model)
        {
            if(!string.IsNullOrEmpty(model.BarcodeID))
            {
                ...
                try
                {
                    ...
                    _assignBox = repo.AssignBoxRepository.FindAssignedBoxBarcodeByBarcodeText(model.BarcodeID);
                    if (_assignBox == null)
                    {
                        long barcodeID = Convert.ToInt64(model.BarcodeID) / 5000;


                        _assignBox = repo.AssignBoxRepository.Find(barcodeID);
                    }
                    ...
                    if (_assignBox != null)
                    {
                        ...
                        outputModel.isLegal = _assignBox.IsLegalHold == true ? "Yes" : "No";
                        outputModel.titleOfContent = _assignBox.Item.Description;
                        outputModel.category = _assignBox.Item.Category == null ? "NA" : _assignBox.Item.Category  ;
        ...
        public ActionResult GetAddBoxDocbyDoc(string documentName)
        {
            if (System.Web.HttpContext.Current.Session["ClientId"] != null)
            {
                clientId = Convert.ToInt64(System.Web.HttpContext.Current.Session["ClientId"]);
            ...
            var deptName = repo.DepartmentRepository.GetDepartmentByUser(userId);
            var box = repo.ItemRepository.getAllAddboxDoc(documentName, deptName.DepartmentName);
            var boxDescription = string.Empty;
            if (box != null)
            {
                boxDescription = repo.ItemRepository.GetByCliendIDandDeptIdandBoxNameandBoxNo(clientId, deptName.DepartmentID, box.BoxName, box.BoxNo).Description;
            }
            ViewBag.BoxDescription = boxDescription;
            ViewBag.newModel = box;
            return View("BoxInfoByBarcode");
        }
        ...
        /*
                public ActionResult ExportData(DateTime startDate, DateTime endDate)
                {
                    ...
                    var data = repo.BoxLocationRepository.FindAllBoxData(startDate, endDate ,clientUser.DepartmentID , subDeptName);
        ...
        public ActionResult ExportData(DateTime startDate, DateTime endDate)
        {
            ...
            // Use clientId instead of departmentId and subDeptName
            var data = repo.BoxLocationRepository.FindAllBoxData(startDate, endDate, clientUser.ClientId);
            ...
            using (var excelConnection = new OleDbConnection($"Provider=Microsoft.ACE.OLEDB.12.0;Data Source={filePath};Extended Properties='Excel 12.0 Xml;HDR=YES;';"))
            {
                ...
                // Create a new Excel sheet and define the schema based on the SQL query fields
                // Fixed the CREATE TABLE syntax by removing extra whitespace and ensuring proper formatting
                string createTableQuery = @"
                CREATE TABLE [Sheet1] 
                (
                    [Box Name] MEMO,
                    [Box No] MEMO,
                    [Year] NVARCHAR(255),      
                    [Destruction Period] DATETIME, 
                    [Legal Hold] NVARCHAR(3),
                    [Assign Date] DATETIME,        
                    [Description] MEMO,
                    [Client Name] NVARCHAR(255),
                    [Department Name] NVARCHAR(255), 
                    [Sub-Department] NVARCHAR(255)
                )";
                ...
                foreach (var box in data)
                {
                    string insertQuery = @"
                    INSERT INTO [Sheet1] 
                    ([Box Name], [Box No], [Year], [Destruction Period], [Legal Hold], [Assign Date], [Description], [Client Name], [Department Name], [Sub-Department]) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

                    using (var cmdInsert = new OleDbCommand(insertQuery, excelConnection))
                    {
                        cmdInsert.Parameters.AddWithValue("@Box Name", string.IsNullOrEmpty(box.BoxName) ? DBNull.Value : (object)box.BoxName);
                        cmdInsert.Parameters.AddWithValue("@Box No", string.IsNullOrEmpty(box.BoxNo) ? DBNull.Value : (object)box.BoxNo);
                        cmdInsert.Parameters.AddWithValue("@Year", string.IsNullOrEmpty(box.Year) ? DBNull.Value : (object)box.Year);
                        cmdInsert.Parameters.AddWithValue("@Destruction Period", box.DestructionPeriod ?? (object)DBNull.Value);
                        cmdInsert.Parameters.AddWithValue("@Legal Hold", string.IsNullOrEmpty(box.Legalhold) ? DBNull.Value : (object)box.Legalhold);
                        cmdInsert.Parameters.AddWithValue("@AssignDate", box.AssignDate.ToString("yyyy-MM-dd HH:mm:ss"));
                        cmdInsert.Parameters.AddWithValue("@Description", string.IsNullOrEmpty(box.Description) ? DBNull.Value : (object)box.Description);
                        cmdInsert.Parameters.AddWithValue("@Client Name", string.IsNullOrEmpty(box.ClientName) ? DBNull.Value : (object)box.ClientName);
                        cmdInsert.Parameters.AddWithValue("@Department Name", string.IsNullOrEmpty(box.DepartmentName) ? DBNull.Value : (object)box.DepartmentName);
                        cmdInsert.Parameters.AddWithValue("@Sub-Department", string.IsNullOrEmpty(box.SubDeptName) ? DBNull.Value : (object)box.SubDeptName);
