...
{ 
    public class ItemRepository : IItemRepository
    {
        ...
        //public List<Item> GetItemUsingItemNoss(long _itemNo)
        //{
        //    List<Item> items = new List<Item>();
        //    items = context.Items.Where(i => i.ItemId == _itemNo).ToList();// && i.DepartmentID == _deptIdc
        //    return items;
        //}

        public Item GetItemUsingItemNo(long _itemNo)
        {
            List<Item> allBox = new List<Item>();
            allBox = context.Items.Where(i => i.ItemId == _itemNo).ToList();

            Item box = new Item();


            box = allBox.FirstOrDefault();

            // box = allBox.Where(s => s.BoxNo.Trim() == _boxNo.Trim() && s.ItemName.Trim() == _itemName.Trim()).FirstOrDefault();

            return box;// context.Items.Where(i => i.BoxNo == _boxNo && i.ItemName == _itemName && i.Year == _year && i.ClientID == _clientID && i.DepartmentID == _departmentID).FirstOrDefault();
        }
        ...
        {
            List<Item> allBox = new List<Item>();
            allBox = context.Items.Where(i =>i.ClientID == _clientID && i.DepartmentID == _departmentID).ToList();

            Item box=new Item();

            box = allBox
                .Where(s => s.BoxNo.Trim() == _boxNo.Trim()
                    && (s.ItemName ?? "").Trim() == (_itemName ?? "").Trim()
                    && (string.IsNullOrEmpty(_year) || (s.Year ?? "").Trim() == _year.Trim()))
                .FirstOrDefault();
        ...
        internal Item GetByCliendIDandDeptIdandFileNameandRingNoandAccountNoAndYear(long _tempClientID, long _tempDepartmentID, string bookNo, string ringNo, string accNo, string year)
        {
            return context.Items.Where(s => s.ClientID == _tempClientID && s.DepartmentID == _tempDepartmentID && s.FileBoxName == bookNo && s.RingNo == ringNo && s.AccountNo == accNo && s.Year == year).FirstOrDefault();
        }
        internal Item GetByCliendIDandDeptIdandFileNameandRingNoandAccountNoAndYearAndFileNo(long _tempClientID, long _tempDepartmentID, string bookNo, string ringNo, string accNo, string year,string fileNo)
        {
            return context.Items.Where(s => s.ClientID == _tempClientID && s.DepartmentID == _tempDepartmentID && s.FileBoxName == bookNo && s.RingNo == ringNo && s.AccountNo == accNo && s.Year == year && s.FileNumber == fileNo).FirstOrDefault();
        }


        internal Item GetByCliendIDandDeptIdandFileNameandRingNoandAccountNo(long _tempClientID, long _tempDepartmentID, string bookNo, string ringNo, string accNo)
        {
            return context.Items.Where(s => s.ClientID == _tempClientID && s.DepartmentID == _tempDepartmentID && s.FileBoxName == bookNo && s.RingNo == ringNo && s.AccountNo == accNo).FirstOrDefault();
        }

        internal Item GetByCliendIDandDeptIdandFileNameandRingNoandAccountNoandFileNo(long _tempClientID, long _tempDepartmentID, string bookNo, string ringNo, string accNo, string fileNo)
        {
            return context.Items.Where(s => s.ClientID == _tempClientID && s.DepartmentID == _tempDepartmentID && s.FileBoxName == bookNo && s.RingNo == ringNo && s.AccountNo == accNo && s.FileNumber == fileNo ).FirstOrDefault();
        }

        internal List<Item> GetDestructionPeriodByMonthCountwithClient(int monthCount, long clientId)
        {
            DateTime monthStart = DateTime.Now;
            DateTime monthEnd = monthStart.AddMonths(monthCount).AddDays(-1);
