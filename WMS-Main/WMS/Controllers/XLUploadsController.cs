...
{
    public class XLUploadsController : Controller
    {
        ...
        public ActionResult XLUpload()
        {
            if (TempData["Flag"] != null)
            {
                ...
                {
                    string count = "0";
                    if (Session["TotalDataUploadedDuplicate"] != null)
                    {
                        count = Session["TotalDataUploadedDuplicate"].ToString();
                    }

                    ViewBag.Flag = 0;
                    if (Session["ErrorMessage"].ToString() == "This XL File Contains Duplicate Data")
                        ViewBag.msg = "This XL File Contains Duplicate Data.Data From 1st Row to " + count + " has been uploaded.Then We Found Duplicate.Please Remove these data along with Duplicate.Then Re Upload Again";
                    else
                        ViewBag.msg = Session["ErrorMessage"].ToString();

                    ViewBag.flagError = 0;
                    if (Session["ErrorMessage"].ToString() == "Please Select Department and Client")
                        ViewBag.msg = "Please Select Department and Client";
        ...
        [HttpPost]
        public ActionResult XLUploadwithFile(HttpPostedFileBase filename, XLUploadViewModel model)
        {

            if (model.ClientId1 > 0)
            {
                ...
                if (filename != null && filename.ContentLength > 0)
                {
                    ...
                    bool flag = Import_To_Grid(path, model, Extension);
        ...
        [HttpPost]
        public ActionResult XLUploadwithFileForFile(HttpPostedFileBase filenameFile, XLUploadViewModel model)
        {
            if (model.ClientId2 > 0)
            {
                ...
                if (filenameFile != null && filenameFile.ContentLength > 0)
                {
                    ...
                    bool flag = Import_To_GridFile(path, model, Extension);
        ...
        [HttpPost]
        public ActionResult XLUploadwithFileForPalletBox(HttpPostedFileBase filenamePallet, XLUploadViewModel model)
        {
            if (model.DeptId3 > 0 && model.ClientId3 > 0)
            {
                ...
                if (filenamePallet != null && filenamePallet.ContentLength > 0)
                {
                    ...
                    bool flag = Import_To_GridPallet(path, model, Extension);
        ...
        private bool Import_To_Grid(string FilePath, XLUploadViewModel model, string Extension)
        {
            bool flag = false;
        ...
        private bool Import_To_GridFile(string FilePath, XLUploadViewModel model, string Extension)
        {
            bool flag = false;
        ...
        private bool Import_To_GridPallet(string FilePath, XLUploadViewModel model, string Extension)
        {
            bool flag = false;
        ...
        private void SaveData(DataTable dt, XLUploadViewModel model)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                string subDepartment = dr["Sub Department"].ToString();
                ...
                if (!string.IsNullOrWhiteSpace(barcodeText) && barcodeText.Length < 10)
                {
                    barcodeText = barcodeText.PadLeft(10, '0');
                }
                bool isLegalHold = dr["Legal Hold"].ToString().ToLower().Trim() == "yes" ? true : false;
                string description = dr["Title of Content"].ToString();
                ...
                try
                {
                     ...
                     destructionDate = Convert.ToDateTime(dr["Distruction Date"].ToString());
                }
                catch
                {
                ...
                long _tempClientID = model.ClientId1;
                long _tempDepartmentID = model.DeptId1;
                long _tempSubDepartmentID = 0;
                long _tempContactPersonID = 0;
                long _tempClientDepartmentID = 0;
                #endregion
                ...
                #region Client Department (For test purpose)
                Department _cltDept = repo.DepartmentRepository.GetByClientIDandDeptName(_tempClientID, department);

               if (_cltDept == null)
                {
                    _cltDept = new Department();
                    _cltDept.DepartmentName = department;
                    _cltDept.DepartmentCode = "";
                    _cltDept.ClientID = _tempClientID;
                    repo.DepartmentRepository.InsertOrUpdate(_cltDept);
                    repo.DepartmentRepository.Save();
                }
               _tempDepartmentID = _cltDept.DepartmentID;
                #endregion
                ...
                {
                    throw new Exception("IssuedBY can not be Empty.");
                }
                ...
                if (_contactPerson == null)
                {
                    ...
                    if (!string.IsNullOrEmpty(contactPerson))
                        ...
                        _contactPerson.ContactPersonName = "NA";
                    _contactPerson.DepartmentID = _tempDepartmentID;
                    _contactPerson.ClientID = _tempClientID;
                    _contactPerson.Client = repo.ClientRepository.Find(_tempClientID);
                    _contactPerson.Department = repo.DepartmentRepository.GetByClientIDandDepId(_tempClientID, _tempDepartmentID);
                    _contactPerson.Address = "NA";
                    _contactPerson.Email = "NA";
                    _contactPerson.Position = "NA";
                ...
                {
                    box = repo.ItemRepository.GetByCliendIDandDeptIdandBoxNameandBoxNoAndYear(_tempClientID, _tempDepartmentID, boxName, boxNo, year);
                }
                else
                {
                    box = repo.ItemRepository.GetByCliendIDandDeptIdandBoxNameandBoxNo(_tempClientID, _tempDepartmentID, boxName, boxNo);
                }
                ...
                if (box == null)
                {
                    ...
                    box.SubDepartment = subDepartment;
                    box.IsArchieve = false;
                    box.ItemName = boxName;
                    box.Quantity = "1";
                    box.Unit = "Box";
                    box.Year = year;
                    if (destructionDate.Date == DateTime.Now.Date)
                    {
                    }
                    else
                    {

                        box.DestructionPeriod = destructionDate;
                    }
                    box.BarCodeText = barcodeText;
                    box.IsLegalHold = isLegalHold;
                    ...
                    assignBox.BarCodeText = box.BarCodeText;
                    assignBox.IsLegalHold = box.IsLegalHold;
                    ...
                    else
                    {
                        ...
                        assignBox.DestructionPeriod = destructionDate;
                    }
                    ...
                    _existingBarCode.ClientName = repo.ClientRepository.Find(assignBox.Item.ClientID).ClientName;// assignBox.Item.Client.ClientName;
                    _existingBarCode.DeptName = repo.DepartmentRepository.Find(assignBox.Item.DepartmentID).DepartmentName;// assignBox.Item.Department.DepartmentName;
                    ...
                    _existingBarCode.ItemId = assignBox.ItemId;
                    ...
                    assignBox.TransmittalINStatusId = Convert.ToInt64(EnumHelper.Status.Location_Assigned);
                    repo.AssignBoxRepository.InsertOrUpdate(assignBox);
                    repo.AssignBoxRepository.Save();
                ...
                {
                    Session["ErrorMessage"] = "This XL File Contains Duplicate Data";
                    break;
                }
            }

            Session["TotalDataUploaded"] = intCounr;
            Session["TotalDataUploadedDuplicate"] = itemCount;
        }
        ...
        private void SaveDataFile(DataTable dt, XLUploadViewModel model)
        {
            List<Item> itemList = new List<Item>();
            long intcount = 0;
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                long _tempORBLDeptID = 0;
                long _tempSupportStuffID = 0;
                long _tempClientID = model.ClientId2;
                long _tempDepartmentID = model.DeptId2;
                //long _tempSubdepartment = 0; 
                long _tempContactPersonID = 0;
                #endregion
               ...
               _tempDepartmentID = _cltDept.DepartmentID;
                ...
                //if (!string.IsNullOrEmpty(year))
                //{
                //    box = repo.ItemRepository.GetByCliendIDandDeptIdandFileNameandRingNoandAccountNoAndYear(_tempClientID, _tempDepartmentID, bookNo, ringNo, accNo, year);
                //}
                //else
                //{
                //    box = repo.ItemRepository.GetByCliendIDandDeptIdandFileNameandRingNoandAccountNo(_tempClientID, _tempDepartmentID, bookNo, ringNo, accNo);
                //}

                //Added by Anik on 04Feb2020
                if (!string.IsNullOrEmpty(year) && !string.IsNullOrEmpty(fileNo))
                {
                    box = repo.ItemRepository.GetByCliendIDandDeptIdandFileNameandRingNoandAccountNoAndYearAndFileNo(_tempClientID, _tempDepartmentID, bookNo, ringNo, accNo, year,fileNo);
                }
                else
                {
                    box = repo.ItemRepository.GetByCliendIDandDeptIdandFileNameandRingNoandAccountNoandFileNo(_tempClientID, _tempDepartmentID, bookNo, ringNo, accNo, fileNo);
                }
        ...
        private void SaveDataPallet(DataTable dt, XLUploadViewModel model)
        {
            List<Item> itemList = new List<Item>();
            long intcount = 0;
            foreach (DataRow dr in dt.Rows)
            {
                ...
                long _tempORBLDeptID = 0;
                long _tempSupportStuffID = 0;
                long _tempClientID = model.ClientId3;
                long _tempDepartmentID = model.DeptId3;
                long _tempContactPersonID = 0;
        ...
        public ActionResult UploadDept()
        {
            ViewBag.PossibleClients = repo.ClientRepository.AllIncluding();
            return View();
        }
        ...
        [HttpPost]
        public ActionResult UploadDept(HttpPostedFileBase filename, XLUploadViewModel model)
        {
            if (filename != null && filename.ContentLength > 0)
            {
                // extract only the fielname
                ...
                bool flag = Import_To_GridDept(path, model, Extension);
            ...
            return RedirectToAction("UploadDept", "XLUploads");

        }

        private bool Import_To_GridDept(string FilePath, XLUploadViewModel model, string Extension)
        {
            bool flag = false;
        ...
        private void SaveDataDept(DataTable dt, XLUploadViewModel model)
        {

            long clientId = model.ClientId1;
            Client client = repo.ClientRepository.Find(clientId);
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                dept.DepartmentAddress = "NA";
                dept.DepartmentCode = "NA";
                dept.DepartmentName = DepartmentName;
                repo.DepartmentRepository.InsertOrUpdate(dept);
                repo.DepartmentRepository.Save();
